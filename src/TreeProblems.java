...
public class TreeProblems {
  ...
  public static int sumTree(Node<Integer> root) {
    ...
    if (root.children != null) {
      for (Node<Integer> child : root.children) {
        sum += sumTree(child); // Recursively add child values
      }
    }
    return sum;
  }
  ...
  */
  public static <T> T findRoot(Map<T, List<T>> tree) {
    ...
    for(T key: tree.keySet()) {
      if(!allChildren.contains(key)) root = key;
    }
    return root;
  }
  ...
  */
  ...
  public static int maxDepthHelper(Map<String,List<String>> tree, String current) {
    if(current == null || !tree.containsKey(current)) return 0;
